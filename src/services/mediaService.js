...
const END_POINTS = {
  ...
  trendingList: (type, page) => `https://api.themoviedb.org/3/trending/${type}/day?&language=es-DO&append_to_response=images&page=${page}`,
  general: (page) => END_POINTS.trendingList('all', page),
  movie: (page) => END_POINTS.trendingList('movie', page),
  tv: (page) => END_POINTS.trendingList('tv', page),
  images: (query, type) => `https://api.themoviedb.org/3/${type}/${query}/images`
}

export async function searchMedia ({ search, type, page }) {
  let response
  if (search !== '' && search !== undefined) {
    ...
    const endpointFunction = END_POINTS[type] || END_POINTS.general
    response = await fetch(endpointFunction(page), options)
  }
...
export async function getMediaInfo (searchParameter) {
  ...
  return {
    ...
    similar: mapMedia(movieInfo.similar.results.slice(0, 8)),
...
function genderMatcher (id) {
  const genresDictionary = {
    ...
    9648: 'Misterio',
    10764: 'Reality',
    10765: 'Ciencia Ficcion & Fantasia',
    10766: 'Telenovela',
    10767: 'TalkShow',
